...
import static com.github.castorm.kafka.connect.http.response.jackson.JacksonRecordKvRecordHttpResponseParserTest.Fixture.bytes;
import static com.github.castorm.kafka.connect.http.response.jackson.JacksonRecordKvRecordHttpResponseParserTest.Fixture.record;
import static com.github.castorm.kafka.connect.http.response.jackson.JacksonRecordKvRecordHttpResponseParserTest.Fixture.response;
import static com.github.castorm.kafka.connect.http.response.jackson.JacksonRecordKvRecordHttpResponseParserTest.Fixture.timestamp;
import static com.github.castorm.kafka.connect.http.response.jackson.JacksonRecordKvRecordHttpResponseParserTest.Fixture.timestampIso;
...
class JacksonRecordKvRecordHttpResponseParserTest {

    JacksonRecordKvRecordHttpResponseParser parser;

    @Mock
    JacksonRecordKvRecordHttpResponseParserConfig config;
    ...
    @BeforeEach
    void setUp() {
        parser = new JacksonRecordKvRecordHttpResponseParser(__ -> config);
    ...
    void givenOneItem_thenTimestampMapped() {
        ...
        assertThat(parser.parse(response))
                .first()
                .extracting(KvRecord::getOffset)
                .extracting(Offset::getTimestamp)
    ...
    void givenOneItem_thenOffsetMapped() {
        ...
        assertThat(parser.parse(response).stream()
                        .findFirst()
                        .get()
                        .getOffset()
                        .toMap()
                        .get("offset-key"))
                .isEqualTo("offset-value");
    }
    ...
    void givenOneItem_thenTimestampMappedToOffset() {
        ...
        assertThat(parser.parse(response).stream().findFirst().get().getOffset().getTimestamp())
                .contains(parse(timestampIso));
    }
    ...
    void givenOneItem_thenKeyMappedToOffset() {
        ...
        assertThat(parser.parse(response).stream().findFirst().get().getOffset().getKey())
                .contains("value");
    }
    ...
    void givenOneItemWithNoKey_thenConsistentUUIDMappedToOffset() {
        ...
        assertThat(parser.parse(response).stream().findFirst().get().getOffset().getKey())
                .contains(nameUUIDFromBytes(record.getBody().toString().getBytes())
                        .toString());
    }
