...
class ConfigurableHttpAuthenticatorConfigTest {
    ...
    void whenNoneType_thenBasic() {
        assertThat(config(ImmutableMap.of("http.auth.type", "None")).getAuthenticator())
                .isInstanceOf(NoneHttpAuthenticator.class);
    }

    @Test
    void whenBasicType_thenBasic() {
        assertThat(config(ImmutableMap.of("http.auth.type", "Basic")).getAuthenticator())
