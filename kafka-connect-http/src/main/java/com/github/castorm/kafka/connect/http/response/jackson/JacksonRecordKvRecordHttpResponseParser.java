...
public class JacksonRecordKvRecordHttpResponseParser implements KvRecordHttpResponseParser {

    private final Function<Map<String, ?>, JacksonRecordKvRecordHttpResponseParserConfig> configFactory;

    private JacksonResponseRecordParser responseParser;

    private TimestampParser timestampParser;

    public JacksonRecordKvRecordHttpResponseParser() {
        this(JacksonRecordKvRecordHttpResponseParserConfig::new);
    }

    @Override
    public void configure(Map<String, ?> configs) {
        JacksonRecordKvRecordHttpResponseParserConfig config = configFactory.apply(configs);
    ...
    public List<KvRecord> parse(HttpResponse response) {
        return responseParser.getRecords(response.getBody()).map(this::map).collect(toList());
    }
